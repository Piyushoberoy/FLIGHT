        ...
        input.addEventListener('input', () => {
            ...
            if(input.classList.contains('pswd')){
                ...
                if(input.value.trim().length !== 0) {
                    if (input.value.trim().length >= 8 && checkPasswordStrength(input.value.trim())) {
                        check[3] = true;
                        input.parentElement.querySelector('span').innerText = "";
                    } else {
                        input.parentElement.querySelector('span').innerText = "Password must be at least 8 characters long and contain at least one uppercase letter, one lowercase letter, one digit, and one special character.";
                        check[3] = false;
                    }
                }
                else{
            ...
            if(input.classList.contains('cpswd')){
                if(input.value.trim().length !== 0) {
                    if(input.value !== document.querySelector('.pswd').value) {
                        input.parentElement.querySelector('span').innerText = "Confirmation Password not matched.";
                        check[4] = false;
                    }
                    else {
                        input.parentElement.querySelector('span').innerText = "";
